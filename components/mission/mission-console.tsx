...
interface MissionUiState {
  ...
  completedPlannedTools: string[];
  report: MissionFinalReport | null;
  error: string | null;
}
  ...
  switch (action.type) {
    ...
    case "ingest_event": {
      ...
      if (event.type === "step_completed") {
        const expectedTools = state.plan
          .map((planStep) => planStep.expectedTool)
          .filter((tool): tool is string => Boolean(tool));
        const isExpected = expectedTools.includes(event.stepName);
        const alreadyIncluded = state.completedPlannedTools.includes(event.stepName);
        const completedPlannedTools =
          isExpected && !alreadyIncluded
            ? [...state.completedPlannedTools, event.stepName]
            : state.completedPlannedTools;
...
export function MissionConsole(): React.JSX.Element {
  ...
  const progress = useMemo(() => {
    const expected = state.plan.filter((step) => step.expectedTool).length;
    if (expected === 0) return state.status === "completed" ? 100 : 0;
    return Math.round((state.completedPlannedTools.length / expected) * 100);
  }, [state.completedPlannedTools.length, state.plan, state.status]);
      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-6">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="min-w-[220px] flex-1">
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  Progress: {progress}% ({state.completedPlannedTools.length}/
                  {state.plan.filter((step) => step.expectedTool).length || 0} planned ERP
                  steps completed)
                </p>
              </div>
            </div>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <Card className="min-h-[460px]">
            <CardHeader>
              ...
              <CardDescription>
                Ordered sub-steps generated by the AI agent.
              </CardDescription>
            </CardHeader>
            <CardContent>
              {state.plan.length === 0 ? (
                <p className="text-sm text-muted-foreground">
                  Plan will appear after mission initialization.
                </p>
              ) : (
                    ...
                    <li key={step.id} className="rounded-lg border p-3">
                      ...
                      {step.expectedTool ? (
                        <p className="mt-2 text-xs text-muted-foreground">
                          Tool: <span className="font-mono">{step.expectedTool}</span>
                        </p>
                      ) : null}
                    </li>
                  ))}
        ...
        {state.report ? (
          <Card>
            ...
            <CardContent className="space-y-4">
              <p className="text-sm">{state.report.executiveSummary}</p>
              <div className="grid gap-4 md:grid-cols-2">
                <div>
                  <h3 className="text-sm font-semibold">Outputs</h3>
                  <ul className="mt-2 list-disc space-y-1 pl-4 text-sm text-muted-foreground">
                    {state.report.outputs.map((item) => (
                      <li key={item}>{item}</li>
                    ))}
                  </ul>
                </div>
                <div>
                  ...
                  <ul className="mt-2 list-disc space-y-1 pl-4 text-sm text-muted-foreground">
                    {state.report.nextActions.map((item) => (
                      <li key={item}>{item}</li>
                    ))}
                  </ul>
                </div>
